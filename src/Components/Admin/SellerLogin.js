...
function SellerLogin() {
    ...
    const login = async()=>{
        ...
        try {
            const res = await axios.post('http://127.0.0.1:8000/seller/login/', formData,
             {
              headers: {
            ...
            <div className="login-container p-4 border rounded" style={{ maxWidth: "400px", width: "100%" }}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group mt-3 text-center">
                        <Link to="/forgotseller" className="text-muted">Forgot Password?</Link>
                    </div>
                    <div className="form-group text-center">
                        <p className="mb-0">Don't have an account? <Link to="/registerseller" className="text-primary">Register</Link></p>
                    </div>
                </form>
            </div>
        </div>
